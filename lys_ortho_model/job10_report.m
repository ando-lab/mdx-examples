...
reportDir = 'report';
reportFileName = 'report_lys_ortho_model.md';
mkdir(reportDir)

%%
% initialize markdown file

report_md = {'# Report: orthorhombic lysozyme modeling',datestr(now)};

%% 

load proc/reference_halos.mat I sigma
fh = figure(1);clf;fh.Position(3:4) = [425,200];
...
imagesc(cell2mat(reshape(m,10,[])),[0,10*std(I(~isnan(I)))]);
set(gca,'Position',[0,0,1,1]);
axis image;axis off;colormap turbo

figName = 'reference_halos_lys_ortho.png';
...
Ilatt = MT.read_data('export/lys_ortho_goodvibes.h5','supercell','I');
...
figName = 'intensity_stats_lys_ortho.png';
exportgraphics(fh,fullfile(reportDir,figName));
...
Tdb = readtable('export/lys_ortho_goodvibes_com_covariances.csv');
...
figName = 'goodvibes_joint_adps_lys_ortho.png';
exportgraphics(fh,fullfile(reportDir,figName));
...
Tdb = readtable('export/lys_ortho_discoball_joint_adps.csv');
T = preprocess_covariance_table(Tdb);
...
figName = 'discoball_joint_adps_lys_ortho.png';
exportgraphics(fh,fullfile(reportDir,figName));
...
Tdb = readtable('export/lys_ortho_discoball_joint_adps.csv');
Tgv = readtable('export/lys_ortho_goodvibes_joint_adps.csv');
...
figName = 'discoball_validation_lys_ortho.png';
exportgraphics(fh,fullfile(reportDir,figName));
...
figName = 'bfactors_lys_ortho.png';
exportgraphics(fh,fullfile(reportDir,figName));
...
mapFileName = 'export/lys_ortho_goodvibes_delta_pdf.h5';
...
mapFileName = '../lys_ortho_map/export/lys_ortho_delta_pdf.h5';
...
mapFileName = 'export/lys_ortho_map_sub.h5';
...
mapFileName = 'export/lys_ortho_goodvibes.h5';
